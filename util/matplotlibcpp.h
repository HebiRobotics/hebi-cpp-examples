...
template <typename Numeric>
PyObject* plot_3dline(const std::vector<Numeric> &x,
                      ...
                      std::map<std::string, std::string>())
{
  ...
  PyObject *kwargs = PyDict_New();
  PyDict_SetItemString(kwargs, "c",PyString_FromString(fmt));
  ...
  PyTuple_SetItem(fig_args, 0, PyLong_FromLong(1));
  PyObject* fig = PyObject_CallObject(detail::_interpreter::get().s_python_function_figure,fig_args);
  Py_DECREF(fig_args);
  ...
  if (!plot) throw std::runtime_error("No surface");
  Py_INCREF(plot);
