...
public:
  ...
  // THIS ENTIRE PART SHOULD BE MOVED UP THIS FILE FOR ORGANIZATIONAL PURPOSES
  ...
  // Get AR Position
  const Vector3f getPosition() const { return fbk_[0].mobile().arPosition().get(); }

  // Get AR Orientation
  const Quaternionf getOrientation() const { return fbk_[0].mobile().arOrientation().get(); }
